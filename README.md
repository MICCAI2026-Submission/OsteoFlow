...
|---|---|---|
| **Teacher (SVF)** | Diffeomorphic registration via stationary velocity fields (SVF) used to generate supervision trajectories during training | Code/OsteoFlow_Teacher_Final.py |
| **Student (Rectified Flow + Lyapunov)** | Image-space rectified flow trained via trajectory distillation with Lyapunov regularization and resection-aware loss | Code/OsteoFlow_Student_Final.py |
...
Illustration of the preprocessing pipeline and the teacher–student distillation framework used to guide the student velocity field.
...
python Code/OsteoFlow_Teacher_Final.py

Run the student model:

python Code/OsteoFlow_Student_Final.py
